...
0.11.1 で「残した」と書いた 2 件を塞いだ。いずれもテストで再現させてから直している。判別できない
書き方は通さず拒否する側に倒している。

**行全体の参照が SELECT の外では oracle になっていた**（R6a）。0.11.1 の検出は SELECT リストの中の
行全体参照だけを見ていた。行の名前では何も返らなくても、どの行がどの順で返るかは行の全列が決めるので、
`WHERE u::text LIKE '%…%'` や `ORDER BY u`、JOIN の `ON`・`GROUP BY`・`HAVING`・サブクエリの中で
行全体を参照すれば、禁止列の値を 1 問ずつ当てられた。計画（UPDATE / DELETE）でも同じで、条件の中の
`WHERE EXISTS (SELECT 1 FROM users u WHERE u::text …)` や `(u.*)::text` は試行実行の可否が答えを返す
oracle になっていた。SELECT リストだけでなく文全体を走査し、禁止列を持つ表を行として参照していれば
（読み取りでも計画でも）実行前に拒否する。

**主キー自体が禁止列のとき、行キーに値がそのまま出ていた**（R6）。DELETE / UPDATE の計画は行を主キーで
識別するが、主キーの生値をそのまま持っていたため、確認カード・保存される計画・適用時の照合メッセージの
すべてに値が載っていた。0.11.1 で禁止列の値に施したのと同じダイジェスト置換を主キーにも適用する。
カードとメッセージには `(withheld by policy)` とだけ出す。適用側は実在行のキーを同じダイジェストに
直してから照合するので、承認した行に正しく当たり、主キーの値は比較にも表示にも出ない。

### 互換性

- 0.11.1 以前で作った計画（主キーが禁止列の表）を 0.11.2 で適用すると、計画側は主キーの生値・適用側は
  ダイジェストで照合するため一致せず、`ROWS_MOVED` で拒否される。計画を作り直すこと。計画側と適用側は
  同時に上げること。
- 禁止列を持つ表に対して、SELECT の外で行全体を参照する読み取り・計画（書き込みでの `*` / `u.*` を含む）は
  拒否する。0.11.1 までは SELECT リストの中だけを検査していた。
- 判別できない書き方は拒否側に倒しているので、表名と同じ綴りの列・別名を SELECT の外で使うと拒否される
  ことがある。
...
外部レビューで指摘されたもの。いずれもテストで再現させてから直した。0.11.0 以前を使っているなら更新を勧める。
判別できない書き方は通さず拒否する側に倒している。

**DELETE の確認カードに禁止列の値が出ていた**（R6）。`denyIdentifiers` に載せた列でも、DELETE の計画は
削除前の行を全列そのまま持ち、カードと保存される計画の両方に値が載っていた。禁止列は値の代わりに
ダイジェストだけを持ち、カードには「列名と、伏せたこと」だけを出す。適用直前の「承認後に値が
変わっていないか」の照合はダイジェストで続けるので、保護は弱まっていない。変わっていた場合の
拒否メッセージにも値は出さない。

**allowlist（R2）を迂回できる表参照の書き方があった**。次の場合に表が検査対象から漏れていた。
- 括弧で囲んだ表参照
- 入れ子の CTE。内側で定義した名前が、スコープの外にある同名の実在表まで隠していた
- 引用符や大文字小文字の違いだけで CTE 名と一致する実在表
- JOIN の `ON` / `USING` 条件の後にカンマで続けた表
- `WINDOW` 句の名前と同名の表

いずれも実在表として照合する。

**PostgreSQL の Unicode エスケープ付き識別子（`U&"…"`）が復号されずに照合されていた**（R6）。
禁止列をエスケープで綴ると `denyIdentifiers` を通過した。復号してから照合する。`UESCAPE` で
エスケープ文字を変える形は拒否する。

**行全体の参照（R6a）の検出漏れ**。括弧・キャスト・演算子で包んだ場合と、カンマ結合した表や
JOIN 全体の別名を行として参照した場合に、行全体の参照と判定されていなかった。禁止列を持つ表なら
実行前に拒否する。

### 不具合修正

**読み取り専用の接続を分けたとき、同時の読み取りが衝突していた**。2 本目の読み取りが同じ接続で
トランザクションを重ねて DB エラーになっていた。実行中なら `BUSY` で拒否する。
計画用の接続を共有している場合の挙動は変わらない。

### 互換性

- 自己参照する CTE は `RECURSIVE` を書くこと。SQLite では省略できるが、省略すると CTE 名が
  実在表として扱われ、allowlist に無ければ `TABLE_NOT_ALLOWED` になる。
- CTE を参照するときは、定義と同じ綴り（引用符と大文字小文字）で書くこと。違う場合は実在表として扱う。
- 0.11.1 で作った DELETE の計画（禁止列を含む表）を 0.11.0 以前で適用すると `ROW_CHANGED` で
  拒否され、そのメッセージには禁止列の現在の値が出る。計画側と適用側は同時に上げること。
  0.11.0 で作った計画は 0.11.1 でもそのまま適用できる。
