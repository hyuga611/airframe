...
`null` や `3` のように JSON としては正しいがオブジェクトでない行があると、`c.probe` を読むところで
例外になっていた。`guard` も Stop フックも exit 70 で終わる。Claude Code では exit 2 以外は止めない
エラーなので、**同じファイルの他の契約ごと、何も確かめずに素通り**していた。

そういう行は `bad-contract` として未達に数え、残りの契約はそのまま確かめる。

### 1 回の実行全体に制限時間を設けた

0.5.4 で probe 1 本ずつには 20 秒の制限を付けたが、probe は順番に走る。固まる probe が 3 本あれば
合計 60 秒になり、Stop フックは Claude Code の timeout（既定 60 秒）で殺される。殺されたフックは
何も止めないので、0.5.4 で塞いだ「固まった probe が合格に見える」穴が、本数を増やすと開いたままだった。

1 回の実行の合計を 45 秒（`GROUNDTRUTH_TOTAL_TIMEOUT_MS` で変更可）とし、各 probe の制限は
「残り時間」までに切り詰める。時間切れで走らせられなかった契約は `out-of-time` として **未達**
に数え、exit 2 で止める。Stop フックと `groundtruth guard` の両方に同じ扱いを入れた。
`checkContract(line, verify, { timeout })` で probe の制限を渡せる。
