...
- ライブラリ本体（`verify` / `gate` / `expect` / `isEmpty` / `GroundtruthIncomplete`）: `src/index.mjs`
- 型定義（consumer 向け・手書き）: `src/index.d.ts`
- CLI（`groundtruth verify` / `groundtruth guard`）: `src/cli.mjs`
...
- **probe を必須にする。** 「行動の戻り値」を証拠として渡すAPIを足さない。これが groundtruth の背骨。
