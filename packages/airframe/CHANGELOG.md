...
### 狭い matcher の下にある手書きのフックを「配線済み」と数えていた

`airframe install` の重複判定はコマンドだけを見て、そのフックがどの matcher の下にあるかを見ていなかった。
たとえば redline の `hook pre` を手で `Write` の下に置いていると、install は何も足さず、
**Bash の呼び出しはリミッターに一度も数えられなかった**。

部品が求める範囲に届かない matcher の下にしか無いときは、そのフックを取り出し、両方の範囲に届く
1 つのグループに移す（部品に matcher が無ければ matcher 無し）。2 つ目のコピーは作らないので、
0.3.1 で入れた「綴りが違っても同じフックは二重に配線しない」はそのまま。同じグループにあった
他のフックは元の matcher に残る。
