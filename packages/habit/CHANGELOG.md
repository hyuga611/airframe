...
### `HABIT_HASH_ONLY=1` でも手直しの本文が保存されていた

`HABIT_HASH_ONLY` は「ファイルの中身を残さない」ための設定だが、効いていたのはファイル本体の保存だけだった。
設定を入れる前に中身を残したファイルがあると、次の書き込みで前回の本文と比べ、
**変更行をそのまま correction に書き出していた**（PostToolUse・PreToolUse の両方）。

本文を残してよいファイルでだけ correction を記録する。PreToolUse で差分を文脈に見せるのは従来どおり。

### 大文字と小文字を同一視するのを Windows と macOS だけにした

ファイルの記録の鍵を作るとき、どの OS でもパスを小文字にしていた。Linux では `A.js` と `a.js` は
別のファイルなので、一方の記録がもう一方で上書きされ、無関係な差分が手直しとして残りうる。

Linux では大文字を含むパスの鍵が変わる。そのため更新直後、そういうファイルへの最初の書き込みは
前回と比べられない（2 回目からは従来どおり）。

## 0.4.4

### 「何を頼まれたか」が長いターンで丸ごと落ちていた

correction には `askedFor`（人が自分の言葉で言った文）が付く。diff は結果で、文は意図。
その文を取る `lastUserMessage()` が、transcript の**末尾 256KB しか読んでいなかった**。

前提が違った。エージェントのターンでは、人の文と実際の書き込みの間に**そのターンの全ツール
呼びと全ツール結果**が挟まる。そして大きいのはそちら。

実測（ローカルの transcript 461本・2075ターン）:

| | |
|---|---|
| 人の発言と ターン末尾の距離が 256KB 超 | **324ターン / 2075（15.6%）** |
| 最大 | **14.7 MB** |
| 人の発言に tool_result と本文が同居していた例 | 0 |

つまり**6ターンに1本、correction が `askedFor: null` で記録される**。意図を剥がされた diff は、
後からルールに蒸留するときの根拠にならない。しかも落ちるのは**長いターン＝最も仕事をした
セッション**の側。

末尾で見つからなければファイル全体（上限 64MB）を読むようにした。全体を読むのは「安い方が
空振りしたとき」＝ correction を書く瞬間だけで、フックのホットパスはここに来ない。

## 0.4.3

### 1152行の `src/habit.mjs` を、自分で引いていた見出しの線で分けた

ファイルの中に `// ---- name ----` の帯が8本あった。書いた本人が「継ぎ目はここだ」と言っていた
ということなので、そこにファイルを置いた。

    store    94   保存先のレイアウトと、パス→キーの変換
    diff     55   2版面 in / 変更行 out ── 唯一の純粋モジュール
    record  298   書き込む3種と、読み戻す全ビュー
    hooks   412   制御を渡される4箇所
    habit   387   入口：CLI と、公開APIの再エクスポート

パッケージの外に出たものは無い。`@hyuga/habit` の API は `src/habit.mjs` から import できるもの
なので、そこから export されていた名前は全部そのまま残っている。既存のテストは1行も変えていない。

### 分けたときに壊れた2箇所を、テストで見つけて直した

どちらも**無言で壊れる**形だった。

- `habit validate --save` が `ensure` / `RULES` / `SAID` / `nowIso` を持たずに残っていた。
  「1 rule(s) kept, 0 dropped」と出したあとで `ensure is not defined` と言い、規則はディスクに
  届かない。しかも **exit code は 1**——証拠が足りず落としたときと同じなので、外からは普通の拒否に見える。
- `hook denied` / `hook failed` が `recordSignal` を持っていなかった。この分岐は
  「フックがユーザーの作業を止めてはいけない」ための `catch {}` の中にあるので、エラーですらなく
  **何も記録しない2つのフック**になっていた。habit の仕事は訂正に気づくことなのに、
  学習元のシグナル2種が、インストールを安全にするためのガードによって捨てられていた。

対策として biome の `correctness/noUndeclaredVariables` を有効にした（既定オフ）。
同じ形が2回出た以上、症状ではなくファイルの中で捕まえる。

### サブコマンドと prune のテストを42本

`src/habit.mjs` は 48.32% で、覆われていない部分は全部サブコマンドだった。中でも `validate` は
**エージェントが作り出した規則と、以後すべてのセッションに注入される文の間に立っている唯一のもの**だ。
存在しない id の引用・訂正1件・同じターンから出た2件・良い規則に混ざった悪い規則1つで
ファイル全体が落ちること（そして良いほうも保存されないこと）を固定した。

`prune` も 74.77% で、覆われていないのは `report()` だった。dry run の出力そのものが
「見せてくれ」と `--apply` の間に立つ最後の1枚なので、そこが未検査なのは順序が逆だった。

### コメントを英語にした

コメント・テスト名・アサーションメッセージ。名前の注（`habit — 習い`）と、
`secrets.mjs` が拾う `パスワード:` のような**実際に人が書く資格情報のラベル**は残している。

## 0.4.2

### 規則を1行に潰してから渡す

規則は、手直しの山からモデルが蒸留した文をファイルに保存したもので、以後すべてのセッション
開始時に文脈へ入る。保存には人が `habit validate --save` を打つ必要があり、証拠のない規則は
そこで落ちる——効いているのはその検査だが、**文そのものは誰かが書いた文ではない**。空行と見出しを
含んだ規則は、ブリーフィングの中の1行ではなく、ブリーフィングの節の顔をして届く。

空白を潰して300字で切るようにした。フレームの台帳と違って引用符では囲まない——この行は従われる
ために置いてあり、従わせたいものを引用符でくくると読みにくくなるだけだから。取り上げたのは
「複数行になれること」だけで、細工に必要なのはそこだった。
